1.选择器的优先级
    选择器的优先级主要考虑权重
    样式表是不同样式方式中的文档内嵌和外链方式代码后来者居上
    可以将各种选择器的权重以数值来表示，数值越大，优先级越高

    选择器　　　　             权值（权重数字化的优先级表示方法）
－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
    标签selector              1
    类选择器                   10
    ID选择器                  100
...
样式的优先级是选择样式的时候行内样式优先级大于文档内嵌或者外链
（文档内嵌和外链优先级根据代码上下顺序）
大于浏览器默认样式大于继承样式
...
-----------------------------------------------
    ...
    2.颜色取值
        ...
        4.颜色的十六进制表示法
            把三原色的十进制范围　0-255转换成十六进制表示　
            十六进制的取值范围　
            0-9 a-f 
            0-255 换算成　16进制　　00-ff
            语法：　
...
3 HTML标签分类
        1.块级元素：
            特点：
                １．独占一行，不与其他元素共行　　拖流变成块元素　，绝对位置脱流
                ２．可以手动调整宽高
                ３．默认情况下，块元素的宽度与父元素保持一致
        ...
        ２．行内元素 :
            特点：
                ...
                ２．不可以手动调整宽高大小
                ３．实际大小由元素内容来决定
            常见行内元素　：　span lable i b s u sup sub strong
...
4 尺寸与边框　
    １．　尺寸
        １．属性
            width height分别设置元素的宽高和大小　
            取值：　像素值或者百分比（百分比是根据父元素的像素值而定）
            注意：　块元素和行内块元素可以设置宽高属性

        ２．溢出处理
            溢出　：　内容超出元素的尺寸范围，称为内容溢出
            默认情况下，溢出部分仍然可见

            属性　: overflow 可以处理内容的溢出
            取值　：　
                1. visible 默认值，溢出部分仍然可见
                2. scroll 　添加滚动条,包含水平和垂直方向
                        　　　不管该方向有没有发生内容溢出，都显示滚动条

                3. auto     自动添加滚动条，哪个方向发生内容溢出
                            添加该方向上的滚动条
    ...
    2.  边框
            ...
            属性　: border  
                取值
                    ...
                    border-color　:　设置边框颜色，取颜色值
                                ...
                                #ffffff
            注意　：　
                    １．使用border属性为元素设置边框，是同时设置　
                        上，　右　下　左　四个方向边框

                    ２．取消默认边框   border:none; (常用于按钮)

        2.  单边边框的设置　：
                属性　：　
                    １．border-top  : 设置顶部边框　
                    ２．border-right : 设置右边边框
                    ３．border-borrom : 设置底部边框
                    ４．border-left : 设置左边边框
        ...
        3.  网页三角标的制作：
            ...
            2 .  为元素设置等宽的边框
            3 ．调增边框颜色，显示一个方向的边框
                其余边框设置透明边框色　transparent

            注意　：　四个方向的边框缺一不可，缺少的话，边框会恢复成矩形边框
                    不再是三角形
...
border-top:20px;
        ...
        5.圆角边框
            ...
            2.取值　：　像素值或者百分比
                ...
                １．border-radius : 20px 
                    一个值表示四个角都以20px做圆角

                ２．border-radius:20px 40px;
                    取两个值　按照上右下左顺时针方向设置圆角，
                    从左上角开始依次取值，在给两个值的情况下，
                    上下保持一致，左右保持一致　

                ３．border-radius: 10px 20px 30px 
        ...
        6 盒阴影
            属性　：　box-shadow 
            取值　：　offset-x offset-y  blur spread color 按顺序书写五个值
                1. offset-x 阴影的水平偏移距离　取像素值
                2. offset-y 阴影的垂直偏移距离，取像素值　
                3. blur 阴影的模糊程 度，取像素值，值越大越模糊
                4. spread 阴影的延伸距离(可省略) 
                5. color : 设置阴影颜色　(默认为黑色)
            注意　：　
                不管是浏览器窗口中还是元素本身，都存在坐标系
                都以左上角为(0,0)点，向右 向下代表x 和　y轴的正方向
